...
public class Start {
	...
	public static void main(String[] args) {
		...
		while (true) {
			...
			switch (opcao) {
			case "1":
				Livro livro = LeitoraDados.cadastrarLivro();
			...
			case "3":
				...
				Caderno caderno = LeitoraDados.cadastrarCaderno();
	...
	private static void identificarUsuario(String cpf) {
		...
		if (resultado.isPresent()) {
			...
			System.out.println("Usuário não cadastrado. Digite novamente ou pressione enter para sair");
			String novoCpf = LeitoraDados.lerDado();
			if (novoCpf.isEmpty()) {
				System.exit(0);// Caso o usuário digite Enter o programa se encerra.
			} else {
				identificarUsuario(novoCpf);// Chama recursivamente o método para validar o CPF novamente
			}
		}
	}
}
